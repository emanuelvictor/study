...
import {Permissao} from "../../../../../../../../domain/entity/permissao.model";
...
// @ts-ignore
@Component({
  selector: 'vincular-permissoes',
  templateUrl: 'vincular-permissoes.component.html',
  styleUrls: ['vincular-permissoes.component.scss']
})
export class VincularPermissoesComponent implements OnInit {
  ...
  @Output()
  save: EventEmitter<Permissao> = new EventEmitter();

  @Output()
  remove: EventEmitter<Permissao> = new EventEmitter();
  ...
  ngOnInit(): void {
    ...
    if (this.permissoes && this.permissoes.length) {
      const data = this.buildFileTree(this.reduce(this.permissoes), 0, false);
  ...
  reduce(array): any {
    return array.reduce((json, value, key) => {
      key = value.nome;
      if (value.permissoesInferiores)
        if (value.permissoesInferiores.length !== 0)
          json[key] = {key: this.reduce(value.permissoesInferiores), id: value.id, selected: value.selected};
        else
  ...
  todoItemSelectionToggle(node: TodoItemFlatNode): void {
    ...
    if (this.checklistSelection.isSelected(node)) {
      this.save.emit(this.findPermissao(this.permissoes, node.id));
    } else this.remove.emit(this.findPermissao(this.permissoes, node.id));
  }
