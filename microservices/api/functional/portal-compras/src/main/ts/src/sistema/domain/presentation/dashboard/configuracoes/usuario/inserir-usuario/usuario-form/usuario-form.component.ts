...
export class UsuarioFormComponent extends CrudViewComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.form = this.fb.group({
      nome: new FormControl({value: '', disabled: this.entity.fornecedor}, Validators.required),
      email: [{value: 'email', disabled: this.entity.fornecedor}, [Validators.required, Validators.email]],
    });
    ...
    this.form.get('email').valueChanges.pipe(
      ...
      .subscribe(usuario => {
        if (usuario) {
          ...
          } else if (!this.entity.fornecedor)
            this.form.controls['nome'].enable();

        } else {

          this.usuarios = [];
          if (!this.entity.fornecedor)
            this.form.controls['nome'].enable();
