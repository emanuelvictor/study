...
import br.org.pti.compras.application.resource.generic.AbstractArquivoResource;
...
import br.org.pti.compras.domain.repository.IFornecedorRepository;
import br.org.pti.compras.domain.repository.dtos.FornecedorWsExterno;
import br.org.pti.compras.domain.repository.dtos.email.AnexoEmail;
...
@RestController
@RequestMapping({FORNECEDOR_MAPPING_RESOURCE, "/sistema/" + FORNECEDOR_MAPPING_RESOURCE})
public class FornecedorResource extends AbstractArquivoResource {
     ...
     */
    @GetMapping
    @PreAuthorize("hasAnyAuthority('" + FORNECEDOR_GET_ROLE + "', '" + ADMINISTRADOR + "')")
    public Page<Fornecedor> listByFilters(final String defaultFilter,
        ...
        return this.fornecedorService.listByFilters(
                ...
                getListFromArray(atividadesEconomicasFilter), getListFromArray(categoriasFilter),
                getListFromArray(estadosFilter), getListFromArray(cidadesFilter),
                pageable
        );
    }
     ...
     */
    @PostMapping
    @PreAuthorize("hasAnyAuthority('" + FORNECEDOR_POST_ROLE + "', '" + ADMINISTRADOR + "')")
     ...
     */
    @PutMapping("/{id}")
    @PreAuthorize("hasAnyAuthority('" + FORNECEDOR_PUT_ROLE + "', '" + ADMINISTRADOR + "')")
    ...
    @PreAuthorize("hasAnyAuthority('" + FORNECEDOR_APPROVE_ACTIVATE_ROLE + "', '" + ADMINISTRADOR + "')")
    ...
    public boolean sendMassMail(@RequestParam("assunto") final String assunto,
        ...
        email.setFornecedores(fornecedorService.listByFilters(null, status, null, null, getListFromArray(categorias), null, null, null).getContent());
